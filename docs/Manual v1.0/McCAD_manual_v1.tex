...
\begin{itemize}
	\item producing a list of solids volumes as loaded from STEP file(s).
	...
	\item producing a mapping of cell IDs on the generated MC input file to the volumes and names of the CAD solids as loaded from the decomposed STEP file(s).
...
\begin{enumerate}
  ...
  \item on McCADInputConfig.i file, the \emph{inputFileName} should be changed to SS\_-793e-2.stp. Also, \emph{convert} and \emph{voidGeneration} parameters should be set to \emph{true}.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \emph{solution}: the direct solution is to carefully eaxmine the decompoed model and look for matching colors between sections where it was suppoed to be split. Manually, in all CAD softwares, using cutting surfaces at the interface, the cylinder and torus can be split and saved along with the decomposed model.
		\begin{figure}[h!]
			\centering
			\begin{subfigure}{.5\textwidth}
				\centering
